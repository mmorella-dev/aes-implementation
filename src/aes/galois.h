...
constexpr auto GaloisMultiply(uint8_t a, uint8_t b) -> uint8_t {
...
consteval auto GenerateLookupTable(const uint8_t n)
    -> std::array<uint8_t, 256> {
  std::array<uint8_t, 256> arr;
  for (int i = 0; i < 256; i++) {
    arr[i] = GaloisMultiply(i, n);
  }
  return arr;
}
/// For a given index i, x2[i] is 2 * i over GN(2^8)
constexpr auto x2 = GenerateLookupTable(2);
/// For a given index i, x3[i] is 3 * i over GN(2^8)
constexpr auto x3 = GenerateLookupTable(3);
/// For a given index i, x2[i] is 9 * i over GN(2^8)
constexpr auto x9 = GenerateLookupTable(9);
/// For a given index i, xB[i] is 11 * i over GN(2^8)
constexpr auto xB = GenerateLookupTable(11);
/// For a given index i, xD[i] is 13 * i over GN(2^8)
constexpr auto xD = GenerateLookupTable(13);
/// For a given index i, xE[i] is 14 * i over GN(2^8)
constexpr auto xE = GenerateLookupTable(14);
}  // namespace galois
